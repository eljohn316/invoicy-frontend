...
import type { DeleteAccountPayload } from '@/features/settings/components/delete-account';
...
export const updateCurrentUserPassword = async (payload: UpdatePasswordPayload) => {
  try {
    ...
    if (isAxiosError(err) && err.code === 'ERR_BAD_REQUEST' && err.response) {
      if (err.status === 400) {
...
export const deleteCurrentUser = async ({ password }: DeleteAccountPayload) => {
  try {
    const { data } = await api.delete<{ message: string }>('/users/current-user', {
      headers: {
